<img src="https://raw.githubusercontent.com/UglyToad/Pdf/master/documentation/pdfpig.png" width="128px" height="128px"/>
...
| Page | Description |
|------|-------------|
| [Home](Home) | This page — overview, installation, and basic usage |
| [PdfDocument](PdfDocument) | Opening documents, passwords, document structure |
| [Letters](Letters) | Letter-level access and text positioning |
| [Images](Images) | Extracting images from PDF pages |
| [Document-Creation](Document-Creation) | Building PDFs with `PdfDocumentBuilder` |
| [Document-Layout-Analysis](Document-Layout-Analysis) | Word extraction, page segmentation, reading order, export |
| [Forms](Forms) | AcroForm interactive form fields |
| [ONNX-Layout-Detection](ONNX-Layout-Detection) | ML-powered layout detection with RT-DETR |
| [Data-Ingestion](Data-Ingestion) | MEDI integration for RAG pipelines |
| [AI-Pipeline-Getting-Started](AI-Pipeline-Getting-Started) | Step-by-step guide from basic extraction to LLM enrichment |
...
See [Document Layout Analysis](Document-Layout-Analysis) for more information on advanced document analysing.

## AI-Powered Features

PdfPig includes companion packages for machine-learning layout detection and RAG pipeline integration:

### Heuristic Page Segmenter

A font-size-aware page segmenter that detects headings and groups text into structured blocks. No ML models required — pure heuristic analysis.

```
> Install-Package UglyToad.PdfPig.DocumentLayoutAnalysis
```

See [Document-Layout-Analysis § Heuristic Page Segmenter](Document-Layout-Analysis#heuristic-page-segmenter) for details.

### ONNX Layout Detection

Deep-learning document layout analysis using ONNX Runtime. Detects 17 region types (text, tables, figures, headers, code, formulas, etc.) using an RT-DETR transformer model.

```
> Install-Package UglyToad.PdfPig.DocumentLayoutAnalysis.Onnx
```

See [ONNX-Layout-Detection](ONNX-Layout-Detection) for model setup, configuration, and GPU support.

### Data Ingestion (MEDI Integration)

First-class integration with [Microsoft.Extensions.DataIngestion](https://learn.microsoft.com/dotnet/ai/conceptual/medi-overview) for building RAG pipelines. Includes LLM-powered processors for table enrichment, OCR fallback, and contextual chunk summaries.

```
> Install-Package UglyToad.PdfPig.DataIngestion
```

See [Data-Ingestion](Data-Ingestion) for pipeline composition and processor docs.

### Getting Started Guide

Not sure which level of PDF processing you need? See the **[AI Pipeline Getting Started](AI-Pipeline-Getting-Started)** guide, which walks through five progressive levels — from basic text extraction to full LLM-enriched RAG pipelines.
...
Please read the [wiki on Images](Images).
