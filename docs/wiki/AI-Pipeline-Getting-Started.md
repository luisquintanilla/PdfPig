...
Building a RAG pipeline?
  └── Yes → Level 4: MEDI Integration
      └── Text-only? → PdfReadingMode.TextOnly (default)
      └── Need vision LLM support? → PdfReadingMode.Hybrid

Need table markdown / OCR fallback / contextual chunks?
  └── Yes → Level 5: LLM Enrichment
      └── Mixed digital + scanned? → PdfReadingMode.Hybrid
      └── Pure VLM processing? → PdfReadingMode.VisionOnly
```
...
- Use `PdfReadingMode.TextOnly` (the default) if you later move to Level 4 but only need text extraction.
...
- Page image rendering is optional at this level — use `PdfReadingMode.Hybrid` in Level 4+ if you plan to use vision LLM processors.
...
// TextOnly is the default — fast text extraction, no page images
...
#### With Hybrid Mode (Page Images)

Use `PdfReadingMode.Hybrid` when you need page images for downstream vision processors:

```csharp
// Hybrid — text extraction + page images + scanned-page placeholders
var reader = new PdfPigReader(
    segmenter: HeuristicPageSegmenter.Instance,
...
// Enable page images with Hybrid mode for downstream vision processors
var reader = new PdfPigReader(segmenter: segmenter, mode: PdfReadingMode.Hybrid);
using var stream = File.OpenRead("document.pdf");
...
- Use `PdfReadingMode.Hybrid` if you plan to use vision LLM enrichment in Level 5 — this enables page image rendering and scanned-page placeholders.
- Use `PdfReadingMode.TextOnly` (the default) for text-only pipelines where you want faster processing and lower memory usage.
...
// Create reader in Hybrid mode for vision processor support
var reader = new PdfPigReader(
    segmenter: HeuristicPageSegmenter.Instance,
    mode: PdfReadingMode.Hybrid,   // text extraction + page images for vision LLMs
    renderDpi: 150);
...
#### VisionOnly Mode (Pure VLM Processing)

For documents where you want a vision language model to handle all content — bypassing native text extraction entirely:

```csharp
// VisionOnly — skip text extraction, render every page, placeholder for every page
var reader = new PdfPigReader(
...
var pipeline = new IngestionPipeline<string>(reader, chunker, writer)
{
    DocumentProcessors =
    {
        new VisionOcrEnricher(chatClient)  // processes every page via vision LLM
    }
};

await pipeline.RunAsync("complex-document.pdf");
```
...
> **Scanned PDF support:** `PdfPigReader` automatically creates placeholder elements (with `Metadata["placeholder"] = true`) for pages that have no extractable text when using `PdfReadingMode.Hybrid`. In `VisionOnly` mode, every page gets a placeholder since text extraction is skipped. This ensures `VisionOcrEnricher` can process scanned/image-only pages via vision LLM OCR — no additional configuration needed. See [Data-Ingestion § Scanned PDF Support](Data-Ingestion#scanned-pdf-support) for details.
...
- Use `PdfReadingMode.Hybrid` for pipelines that combine text extraction with vision LLM enrichment — the processors send multi-content messages with actual page images.
- Use `PdfReadingMode.VisionOnly` for pure VLM processing where you want the vision model to handle all content extraction.
- Use `PdfReadingMode.TextOnly` for text-only pipelines where speed and lower memory usage are priorities.
...
| Level | Package(s) | Adds | Reading Mode | Best For |
|-------|-----------|------|-------------|----------|
| 1 | `PdfPig` | Text extraction | N/A (core API) | Simple text dumps |
| 2 | `PdfPig` | Heading/paragraph detection | N/A (core API) | Structured single-column docs |
| 3 | `PdfPig` + `...Onnx` + ONNX Runtime | 17-class ML layout detection | N/A (core API) | Complex multi-region layouts |
| 4 | `PdfPig` + `...DataIngestion` | MEDI pipeline reader + page images | `TextOnly` or `Hybrid` | RAG pipeline ingestion |
| 5 | `PdfPig` + `...DataIngestion` + `M.E.AI` | Vision table/OCR/context enrichment | `Hybrid` or `VisionOnly` | Production RAG with vision LLMs |
